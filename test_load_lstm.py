...
def train_and_test(model_name = 'LSTM',
    ...
    model.load_state_dict(torch.load(os.path.join(md_path, f'{model_name}_model_seq_{SEQ_LENGTH}_{md_param}.pth')))
    model.eval()
    return testing(model, criterion, test_data, test_labels, len(set(y_cluster)), plotting)


if __name__ == '__main__':
    train_and_test(model_name = 'LSTM', num_layers = 2, hidden_layers=128, remove_min_grp = True, md_param = 'hl_128_lr_0.01')